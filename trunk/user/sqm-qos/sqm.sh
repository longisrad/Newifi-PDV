...
[ -z "$sqm_wan" ] && sqm_wan=$(nvram get wan0_ifname)
[ -z "$sqm_wan" ] && sqm_wan="eth3" # Cổng WAN mặc định phổ biến trên MT7621
...
stop_sqm() {
    logger -t "SQM" "Đang dừng SQM trên cổng $sqm_wan..."
...
start_sqm() {
    ...
    logger -t "SQM" "Đang khởi chạy SQM trên cổng $sqm_wan (Tải về: ${sqm_dl}Kbps, Tải lên: ${sqm_ul}Kbps) thuật toán: $sqm_qdisc..."
    
    # Cấu hình tối ưu băng thông tải lên (Upload) trên giao tiếp WAN
    if [ "$sqm_ul" -gt 0 ]; then
